...
import type { User, StudyPlan, QaMessage, FileItem, LearningProgress, LoginResponse, ApiResponse, CaptchaResponse } from '../types/user'
...
export const api = {
  ...
  logout: () => 
    request<ApiResponse<null>>({ method: 'POST', url: '/logout' }),
  
  refreshToken: () => 
    request<ApiResponse<null>>({ method: 'POST', url: '/token/refresh' }),
  
  sendVerifyCode: (email: string) => 
    request<ApiResponse<null>>({ method: 'POST', url: '/verify/email', data: { email } }),
  
  getCaptcha: () => 
    request<CaptchaResponse>({ method: 'GET', url: '/captcha' }),
  
  // 学习计划模块
  getStudyPlans: () => 
    request<ApiResponse<StudyPlan[]>>({ method: 'GET', url: '/study-plans' }),
  ...
  askQuestion: (question: string) => 
    request<ApiResponse<{ question: string; answer: string }>>({ method: 'POST', url: '/qa/ask', data: { question } }),
  
  getQaHistory: () => 
    request<ApiResponse<QaMessage[]>>({ method: 'GET', url: '/qa/history' }),
  ...
  updateProgress: (data: Partial<LearningProgress>) => 
    request<ApiResponse<LearningProgress>>({ method: 'POST', url: '/learning-progress/update', data })
}
